...
;#3) start_day takes x a given job , l is list of tasks return time that job takes
(defun gettime(x l)
...
(defun start_day(x l)
	...
	((listp x)(+ (gettime (cadr x) l) (get_max (pred (cadr x) l) l)))
	(t (+ (gettime x l) (car (get_max (pred x l) l))))))
...
(cond ((null jobl)  (list (start_day j l) j))
((and (null (cdr jobl)) (> (start_day (car jobl) l) (start_day j l))
(list (start_day (car jobl) l) (car jobl))))
((null (cdr jobl)) (list (start_day j l) j))
((and (< (start_day (car jobl) l)(start_day (cadr jobl) l)) 
(> (start_day (cadr jobl) l) (start_day j l))) (maxhpr (cadr jobl) (cdr jobl) l))
((and (> (start_day (car jobl) l)(start_day (cadr jobl) l)) 
(> (start_day (car jobl) l) (start_day j l))) (maxhpr (car jobl) (cdr jobl) l))
((and (< (start_day (car jobl) l)(start_day (cadr jobl) l))
(< (start_day (cadr jobl) l) (start_day j l))) (maxhpr j (cdr jobl) l))
((and (> (start_day (car jobl) l)(start_day (cadr jobl) l)) 
(< (start_day (car jobl) l) (start_day j l))) (maxhpr j (cdr jobl) l))))
	


(setq tasks '((purchase_lot 2) (design_house 5) (get_permit 1 purchase_lot design_house) 
(get_bids 14 purchase_lot design_house) (select_subs 2 get_bids) (excavate 1 get_permit select_subs)
(construct_basement 7 excavate) (order_windows_doors 3 purchase_lot design_house) 
...
(vapor_barrier_insulation 2 roof install_windows_doors) (drywall 5 vapor_barrier_insulation) 
(inside_paint 3 drywall) (cupboards 3 inside_paint) (carpet_floor 5 inside_paint) 
(lights 2 inside_paint) (plumbing_heating 6 inside_paint) (siding 2 roof install_windows_doors) 
(outside_paint 3 siding) (move_house 1 cupboards carpet_floor lights plumbing_heating outside_paint)
(connections 2 construct_basement move_house) (landscape 4 construct_basement move_house)))
