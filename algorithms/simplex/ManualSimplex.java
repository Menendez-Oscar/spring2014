...
//==================================================================================================
  ...
  {
    ManualSimplex ms = new ManualSimplex("Manual Simplex Method Tool", 0, 0, pixelWidth, pixelHeight );
  }
  ...
  public ManualSimplex( String title, int ulx, int uly, int pw, int ph )
  {
    ...
    System.out.print("enter name of LP file: ");
  ...
  private void toTeX( String fileName )
  {
    try{
      ...
      out.println("\\setcoordinatesystem units <12pt,12pt>");
      ...
      int k;
      for( k=0; k<numRows; ++k )
        out.println("\\put {$" + fix((String) rowLabels[k] ) + "$} [r] at -0.25 " +
                           (h-k-0.5) );

      for( k=0; k<numCols; ++k )
        out.println("\\put {$" + fix((String) colLabels[k]) + "$} [b] at " +
                          (k+0.5) + " " + (h+0.25) );
      ...
      for( int r=0; r<numRows; r++ )
        ...
        {
          if( a[r][c] == (int) a[r][c] )
            out.println("\\put {$" + ((int) a[r][c]) + "$} at " +
                            (0.5+c) + " " + (h-r-0.5) );
          else
            out.println("\\put {$" + a[r][c] + "$} at " +
                            (0.5+c) + " " + (h-r-0.5) );
        }
